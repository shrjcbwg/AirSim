...
void AirsimROSWrapper::initialize_ros()
{
    ...
    vel_cmd_duration_ = 0.01; // todo rosparam
...
void AirsimROSWrapper::create_ros_pubs_from_settings_json()
{
    ...
    for (const auto& curr_vehicle_elem : AirSimSettings::singleton().vehicles) {
        ...
        if (airsim_mode_ == AIRSIM_MODE::DRONE) {
            ...
            drone->vel_cmd_body_frame_sub = nh_private_.subscribe<airsim_ros_pkgs::VelCmd>(curr_vehicle_name + "/vel_cmd_body_frame", 10, boost::bind(&AirsimROSWrapper::vel_cmd_body_frame_cb, this, _1, vehicle_ros->vehicle_name)); // todo ros::TransportHints().tcpNoDelay();
            drone->vel_cmd_world_frame_sub = nh_private_.subscribe<airsim_ros_pkgs::VelCmd>(curr_vehicle_name + "/vel_cmd_world_frame", 10, boost::bind(&AirsimROSWrapper::vel_cmd_world_frame_cb, this, _1, vehicle_ros->vehicle_name));
            drone->vel_cmd_fast_sub = nh_private_.subscribe<quadrotor_msgs::PositionCommand>("/velocity_cmd", 10000, boost::bind(&AirsimROSWrapper::vel_cmd_fast_cb, this, _1, vehicle_ros->vehicle_name));
...
void AirsimROSWrapper::vel_cmd_fast_cb(const quadrotor_msgs::PositionCommand::ConstPtr& msg, const std::string& vehicle_name)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);

    auto drone = static_cast<MultiRotorROS*>(vehicle_name_ptr_map_[vehicle_name].get());

    drone->vel_cmd.x = msg->velocity.x;
    drone->vel_cmd.y = -msg->velocity.y;
    drone->vel_cmd.z = -msg->velocity.z;
    drone->vel_cmd.drivetrain = msr::airlib::DrivetrainType::MaxDegreeOfFreedom;
    drone->vel_cmd.yaw_mode.is_rate = true;
    drone->vel_cmd.yaw_mode.yaw_or_rate = math_common::rad2deg(- msg->yaw_dot);
    drone->has_vel_cmd = true;
}
    ...
    for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
        ...
        if (airsim_mode_ == AIRSIM_MODE::DRONE) {
            ...
            if (drone->has_vel_cmd) {
                std::lock_guard<std::mutex> guard(drone_control_mutex_);
                static_cast<msr::airlib::MultirotorRpcLibClient*>(airsim_client_.get())->moveByVelocityAsync(drone->vel_cmd.x, drone->vel_cmd.y, drone->vel_cmd.z, vel_cmd_duration_, msr::airlib::DrivetrainType::ForwardOnly, drone->vel_cmd.yaw_mode, drone->vehicle_name);
            }
